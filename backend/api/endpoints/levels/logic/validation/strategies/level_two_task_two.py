...
class LevelTwoTaskTwoValidator(IConcreteValidation):
    @inject
    def __init__(self, db: IPostgresqlConnection, db_user_handler: DbUserHandler):
        ...
        self._admin_user: DbUser = self._db_user_handler.get_user_by_username("p_braun")

    async def handle(self, user_uuid: str) -> LevelValidationResult:
        return LevelValidationResult(level="2.2",
                                     is_valid=True,
                                     message="")

    @classmethod
    def can_handle(cls, level_number: int, task_number: int) -> bool:
        return level_number == 2 and task_number == 2
